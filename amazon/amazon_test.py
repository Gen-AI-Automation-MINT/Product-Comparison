from amazon_main import get_amazon_soup, check_amazon_url_status, extract_product_data, amazon_url_list, \
    generate_error_data, STATUS_MAP
...
def process_url(url):
    a_soup_data = get_amazon_soup(url, driver_type='re')
    a_page_status_ = check_amazon_url_status(a_soup_data)
    if a_page_status_ == 200:
        a_product_data = extract_product_data(a_soup_data, url)
        print(product_data)
    else:
        a_product_data = generate_error_data(url, STATUS_MAP.get(a_page_status_, "Unknown"))
    return a_product_data


if __name__ == "__main__":
    amazon_url = amazon_url_list_exa[23]
