...
for i in range(0, 5):
    ...
    if walmart_data[i]['name'] != None and amazon_data[i]['name'] != None:
        ...
        user_content = f"I am providing the JSON data of two products here. Walmart Product = {walmart_product} | Amazon Product = {amazon_product} Your task is to conduct a detailed attribute comparison between the two products, focusing on the following key aspects: Product Name, Brand, Short Description, and Additional Attributes (such as Color, Material, Cost, and Type). Subsequently, present the matching details for my analysis. Your output should deliver a detailed breakdown of the matching attributes, facilitating in-depth analysis. Additionally, provide the percentage match for easier reference. Justify the percentage match by providing a detailed explanation of the matching attributes."
        print("\n\n\n")
        print(user_content)
        body = {
